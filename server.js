	...
	client.on(
		...
		function translate_str_to_unit(name_list, index)
		{
			...
			for(var i = 0; i < name_list.length; ++i)
			{
				...
				{
					case "warrior":
						var unit = {src: "", src_select:"", name:"warrior", ability:"Sweeping Attack", cooldown:1, xcoor:null, ycoor:null, health:16, maxHealth:16, damage:4, maxDamage:4, range:1, movement:2, can_move:true, can_attack:true, is_dead:false, has_been_placed:false, arr_index:i};
		...
		//Set srcs of a priest unit object based on player color
		function checkPlayerPriest(unit, index)
		{
			switch(index)
			{
				case 0: //blue
		...
		//Set srcs of a hunter unit object based on player color
		function checkPlayerPlant(unit, index)
		{
			switch(index)
			{
				case 0: //blue
...
function ability(xcoor, ycoor)
{
	...
	switch(user.ability)
	{
		case "Heal":
			...
			break;
		case "Growth":
			//Max range 2, min range 1, empty space.
